...
const homeProfileBtn = document.querySelector("#homeProfileBtn");
...
homePuzzleBtn?.addEventListener("click", () => {
  playSound("button");
  openStartScreenForMode("practice");
});
...
function renderHomeDashboard() {
  ...
  if (puzzlesProgress) puzzlesProgress.textContent = attempts ? `${solved} solved / ${puzzleAccuracy}%` : "Ready";
  ...
  if (homeStatsGrid) {
    ...
    homeStatsGrid.innerHTML = `
      <div class="home-stat-head">
        <span>Stats</span>
        <strong>${stats.games ? `${stats.wins}-${stats.losses}-${stats.draws}` : "No games yet"}</strong>
        <p>${stats.games ? `${avgAccuracy} average accuracy across ${stats.games} game${stats.games === 1 ? "" : "s"}.` : "Play a game or solve puzzles to fill this in."}</p>
      </div>
...
function renderPuzzleCategoryProgress() {
  ...
  puzzleCategoryProgress.innerHTML = categories
    .map((category) => {
      ...
      return `<article><span>${escapeHtml(category)}</span><strong>${count} puzzle${count === 1 ? "" : "s"}</strong><p>Part of the current tactics path.</p></article>`;
    })
    .join("");
}
...
function renderFriendsOverview() {
  ...
  friendsOverview.innerHTML = `
    ${platformCardMarkup("Friends", friendsState.friends.length, "Players you can invite straight into a match.")}
    ${platformCardMarkup("Requests", incoming + outgoing, `${incoming} incoming / ${outgoing} sent. Refresh to update.`)}
    ${platformCardMarkup("Game Invites", invites, invites ? "You have a friend match waiting." : "No pending game invites.")}
  `;
}
...
function renderProfileOverview(stats = getSavedStats()) {
  ...
  profileOverview.innerHTML = `
    ${platformCardMarkup("Signed In As", name, currentUser ? "Profile changes save to the backend." : "Log in to save your platform data.")}
    ${platformCardMarkup("Local Record", `${stats.wins}-${stats.losses}-${stats.draws}`, "Wins, losses, and draws from this browser.")}
    ${platformCardMarkup("Puzzle Streak", currentUser?.puzzle?.streak ?? 0, "Current account puzzle streak.")}
  `;
}
...
function renderSettingsOverview({ modeLabel, clockLabel, themeLabel, pieceLabel }) {
  ...
  settingsOverview.innerHTML = `
    ${platformCardMarkup("Game Setup", modeLabel, `${clockLabel} / ${difficultySettings[difficultySelect.value]?.botName ?? "Local players"}.`)}
    ${platformCardMarkup("Board", themeLabel, `${pieceLabel} pieces with saved visual preferences.`)}
    ${platformCardMarkup("Helpers", `${isCoachEnabled() ? "Coach on" : "Coach off"} / ${isOpeningEnabled() ? "Openings on" : "Openings off"}`, "Helper settings apply immediately.")}
  `;
}
...
function updateStartFieldsVisibility() {
  const shouldHideBot = startModeSelect.value === "human" || startModeSelect.value === "practice" || startModeSelect.value === "online";
...
function updateStartBotPreview() {
  ...
  startBotAvatar.dataset.bot = isHumanGame || isPractice || isOnline ? "human" : difficulty.theme;
  startBotName.textContent = isPractice ? "Practice Board" : isOnline ? "Online Friend" : isHumanGame ? "Local Players" : difficulty.botName;
  startBotElo.textContent = isPractice ? "Puzzle mode" : isOnline ? "Peer match" : isHumanGame ? "No bot" : difficulty.elo;
...
function getApiBase() {
  return window.localStorage.getItem(settingKeys.apiBase) || "http://localhost:8787";
}
...
function getPuzzleProgressLabel() {
  ...
  return `${difficulty}${level}Puzzle ${currentPuzzleIndex + 1} of ${puzzles.length}`;
}
...
function showPuzzleDialog(message) {
  ...
  if (puzzleDialogSummary) {
    ...
    puzzleDialogSummary.textContent = `${getPuzzleSummary()} · ${solution}`;
  }
