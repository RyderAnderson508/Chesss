...
onlineCopyBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    setOnlineStatus("Code copied. Send it to your friend.");
  } catch {
    setOnlineStatus("Copy failed. Select the code and copy it manually.");
  }
});
...
function sendOnlineMove(review) {
...
function renderOnlineControls() {
  ...
  onlineHostBtn.disabled = !isOnlineMode() || onlineConnected;
  onlineJoinBtn.disabled = !isOnlineMode() || onlineConnected;
  onlineAcceptBtn.disabled = !isOnlineMode() || onlineConnected || onlineRole !== "host";
...
function renderBotCard() {
  ...
  if (isOnlineMode()) {
    ...
    botElo.textContent = onlineConnected ? (onlineLocalColor === "w" ? "You are White" : "You are Black") : "Not connected";
    botStyle.textContent = "Use the Online Friend panel to connect two devices.";
    botChatter.textContent = onlineConnected ? "Live connection ready." : "Trade invite and answer codes to start.";
    return;
  }
  if (isPracticeMode()) {
    const puzzle = getCurrentPuzzle();
